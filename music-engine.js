  ...
  return new Promise(resolve=>{
    ...
    img.onload=()=>{
      ...
      for(let x=0;x<w;x++){
        ...
        const a=lumAt(x,Math.max(0,hr-3));
        const b=lumAt(x,Math.min(h-1,hr+3));
        contrast+=Math.abs(a-b);
      }
      strength=clamp((strength/w)/.22,0,1);
      contrast=clamp((contrast/w)/.35,0,1);
...
function apply(a,b,c,p){
  ...
  // Sustained low-brass horizon: centred near 73 Hz (D2), with only tiny drift.
  const base=73.42 + (0.5-h)*1.2;
  const detune=0.16 + contrast*0.18;
  const cutoff=lerp(420,1050,strength);
  const level=lerp(0.022,0.042,contrast);

  osc1.frequency.setTargetAtTime(base,now,1.8);
  osc2.frequency.setTargetAtTime(base+detune,now,1.8);
  filter.frequency.setTargetAtTime(cutoff,now,1.6);
  gain.gain.setTargetAtTime(level,now,1.7);
...
function updateMonitor(q){
  ...
  parent.innerHTML=`<div><strong>HORIZON — LOW BRASS LONG TONE</strong><br>Frequency ${q.base.toFixed(2)} Hz<br>Horizon ${q.h.toFixed(3)}<br>Line strength ${q.strength.toFixed(3)}<br>Contrast ${q.contrast.toFixed(3)}<br>Timbre ${Math.round(q.cutoff)} Hz<br>Level ${q.level.toFixed(3)}</div>`;
}
...
async function init(){
  ...
  osc1=ctx.createOscillator();osc2=ctx.createOscillator();filter=ctx.createBiquadFilter();gain=ctx.createGain();
  osc1.type='sawtooth';osc2.type='triangle';
  osc1.frequency.value=73.42;osc2.frequency.value=73.58;
  filter.type='lowpass';filter.frequency.value=700;filter.Q.value=.65;
  gain.gain.value=.0001;
  osc1.connect(filter);osc2.connect(filter);filter.connect(gain);gain.connect(master);
  osc1.start();osc2.start();
  await ctx.resume();
  gain.gain.exponentialRampToValueAtTime(.03,ctx.currentTime+1.2);
  running=true;
  await analyseAll();
  applyCurrent();
}
